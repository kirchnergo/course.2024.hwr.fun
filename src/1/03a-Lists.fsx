...
[ 1 .. 10 ] |> List.head
[ 1 .. 10 ]
|> List.sortBy negative
|> List.head
[ 1 .. 10 ]
|> List.filter isGreaterThan5
|> List.head
...
[ 1 .. 10 ]
|> List.map float
|> List.average // How did I convert the ints to floats?
...
let listMatchingExample aList =
    ...
    | first :: rest -> printfn "the list has more than one element and the first element is %i" first
